...
author: "Chief Science Officer Rutvik Gupta; ID: 1004939837"
output:
...
header-includes:    
  ...
  - \usepackage{fancyhdr}
  - \pagestyle{fancy}
  - \fancyhead[LO, LE]{Rutvik Gupta, 1004939837}
...
```{r}
crew_data$name[crew_data$position == "Communications Officer"][0:1]
```
...
```{r}
library(stringr)
crew_data$position[crew_data$position == "Enigneering Technician"] = "Engineering Technician"
eng_sub_division <- crew_data %>% filter(sub_division == "Engineering")
total_eng_division_count <- count(eng_sub_division)
eng_position_count <- count(eng_sub_division %>% filter(str_detect(position, "Engineer")))
ratio <- eng_position_count / total_eng_division_count
ratio
```
...
week1$duty_shift <- factor(week1$duty_shift, c("Gamma", "Delta", "Beta", "Alpha"))
...
```{r}
week1 %>% ggplot(aes(x=duty_shift, y=productivity, color = shift_team)) + 
...
```{r}
crew_data %>% 
  ggplot(aes(x=week, y=productivity, color = division, group=crew_id)) +
...
```{r, eval=FALSE}
model_3 <- lmer(productivity ~ week + starfleet_gpa + perseverance_score + (1 + week|full_team), 
                 data = crew_data)
...
```{r}
library(reshape2)
complete_comp <- melt(competition_data, id.var = "crewmember")
names(complete_comp)[2] <- paste("plant")
names(complete_comp)[3] <- paste("count")
complete_comp <- complete_comp %>% inner_join(origin_data) %>% filter(count >  0) %>% distinct(crewmember, native_to)
planet_systems <- complete_comp %>% group_by(crewmember) %>% summarise(system_planet_count = n())
planet_systems
```
...
#                                      "Delta Quadrant","Telemarius IV",
#                                      "Acamar III","M-113",NA))
# filter(origin_data, nativeto == "Delta Quadrant")
```
